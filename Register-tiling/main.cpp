...
int main() {
    ...
    // Create Kernel and Pass Arguments
    ...
    // Each thread calculates 2x2 = 4 output elements, so launch (N/2, N/2) total threads.
    size_t globalWorkSize[2] = { (size_t)(N / 2), (size_t)(N / 2) };
    size_t localWorkSize[2] = { TILE_SIZE, TILE_SIZE }; // 16x16 work-group = 256 threads

    printf("Executing Level 4 Register-Tiled Ping-Pong Kernel on GPU...\n");
    ...
    double gflops = (2.0 * (double)N * (double)N * (double)N) / (executionTimeMs * 1e6);

    printf("GPU Kernel Execution Time: %.3f ms (%.2f GFLOPS)\n", executionTimeMs, gflops);
